...
Logo → 项目名 → Archimedes / Eureka 核心句 → 三个平台与插件定位 → 安装入口。
中英文可保留各自既有导航与徽章位置，但必须使用相同资产和同义描述。
...
保留痛点、能力、协作模式、安装、用法和架构等现有技术内容。
品牌图片统一放在 `assets/brand/`；流程图、协作图继续放在 `docs/images/`。
